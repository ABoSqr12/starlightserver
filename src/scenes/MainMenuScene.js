...
export class MainMenuScene extends Phaser.Scene {
  ...
  create() {
    ...
    this.#createCredit();
      ...
      this.add.text(this.scale.width / 2, this.scale.height / 2 - 26, 'تعذر تحميل القائمة الرئيسية', { fontFamily: 'Tahoma, Arial, sans-serif', fontSize: '34px', color: '#f2f7ff' }).setOrigin(0.5);
      this.add.text(this.scale.width / 2, this.scale.height / 2 + 24, 'حاول إعادة التحميل', { fontFamily: 'Tahoma, Arial, sans-serif', fontSize: '24px', color: '#d6e6ff' }).setOrigin(0.5);
      console.error(err);
    }
  }
  ...
  update() {
    this.starLayers.forEach((layer) => {
      layer.children.getChildren().forEach((star) => {
        star.x -= layer.speed;
        if (star.x < -3) {
