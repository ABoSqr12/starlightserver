export class SplashScene extends Phaser.Scene {
  ...
  create() {
    ...
    const motto = this.add
      .text(this.scale.width / 2, this.scale.height * 0.43, 'اضغط Enter للمتابعة', {
        fontFamily: 'Tahoma, Arial, sans-serif',
        fontSize: '28px',
        color: '#d2e7ff',
      })
  ...
  #createLightParticles() {
    ...
    for (let i = 0; i < 22; i += 1) {
      ...
      this.glowDots.push({ d, vy: Phaser.Math.FloatBetween(-0.09, -0.03) });
    }
    this.events.on('update', () => {
      this.glowDots.forEach((g) => {
        g.d.y += g.vy;
        g.d.alpha -= 0.004;
        if (g.d.alpha <= 0.05 || g.d.y < 260) {
          g.d.x = Phaser.Math.Between(430, 530);
          g.d.y = Phaser.Math.Between(360, 460);
          g.d.alpha = Phaser.Math.FloatBetween(0.3, 0.8);
        }
      });
    });
  }
